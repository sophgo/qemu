...
RSIZE = 8
CSIZE = 6
os.system("/opt/LLVM/install/bin/clang -static sg_madds.c sg_madds_asm.S -o sg_madds -O3 -march=rv64imafdcv_zicsr_zifencei_zfh_zba_zbb_zvfh_xsfvfnrclipxfqf_xsfvfwmaccqqq_xsfvqmaccqoq_xsfvcp_xsgmat --gcc-toolchain=/opt/riscv --sysroot=/opt/riscv/sysroot/ -g")
...
# data configuration
executable_path = './sg_madds'  # excutable file
...
for i in range(CASE_NUMBER):
    ...
    input1 = np.random.randint(-60, 61, size=(RSIZE, CSIZE), dtype=np.int8)
    input2 = np.random.randint(-60, 61, size=(RSIZE, CSIZE), dtype=np.int8)
    output_matrix_add = np.add(input1, input2).astype(np.int8)
    ...
    if 'sifive-x280' in cpu_list:
        result = subprocess.run(["qemu-riscv64", "-cpu", "sifive-x280", executable_path, input_file_path, str(assemble_64bit(8, 8, 6))], capture_output=True, text=True)
    else:
        result = subprocess.run(["qemu-riscv64", "-cpu", "max,vlen=512", executable_path, input_file_path, str(assemble_64bit(8, 8, 6))], capture_output=True, text=True)
    # exam the output
