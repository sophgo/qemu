...
CASE_NUMBER = 1500
os.system("/opt/LLVM/install/bin/clang -static sg_mconvf.c sg_mconvf_asm.S -o sg_mconvf -O3 -march=rv64imafdcv_zicsr_zifencei_zfh_zba_zbb_zvfh_xsfvfnrclipxfqf_xsfvfwmaccqqq_xsfvqmaccqoq_xsfvcp_xsgmat --gcc-toolchain=/opt/riscv --sysroot=/opt/riscv/sysroot/ -g")

# data configuration
executable_path = './sg_mconvf'  # excutable file
...
for i in range(CASE_NUMBER):
    ...
    padded_input = padded_input.astype(np.float16)
    output_matrix = np.zeros((o_h, o_w), dtype=np.float16)
    for y in range(o_h):
        for x in range(o_w):
            sub_matrix = padded_input[y*strh:y*strh+k_h, x*strw:x*strw+k_w]
            conv_kernel = kernel.astype(np.float16)
