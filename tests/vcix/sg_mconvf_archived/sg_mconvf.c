...
extern void vcix_mconvf_asm(int ih, int iw, int oh, int ow, uint64_t conv, void *input, void *filter, void* output);
void matrix_multiply_and_compare(_Float16 *c, _Float16 *o_c, int m, int n);
...
int main(int argc, char **argv) {
    ...
    _Float16 *output_matrix = (_Float16 *)malloc(sizeof(_Float16) * (int)ow * (int)oh);
    ...
    fread(output_matrix, sizeof(_Float16), (int)ow * (int)oh, input_file);
    fclose(input_file);

    _Float16 *o_c = (_Float16 *)malloc(sizeof(_Float16) * (int)ow * (int)oh);
    
    printf("ih: %d, iw: %d, kh: %d, kw: %d, oh: %d, ow: %d\n", ih, iw, kh, kw, oh, ow);
    vcix_mconvf_asm(ih, iw, oh, ow, conv, input_matrix, kernel, o_c);
...
void matrix_multiply_and_compare(_Float16 *c, _Float16 *o_c, int m, int n) {
    for(int i = 0; i < m; ++i) {
        for(int j = 0; j < n; ++j) {
            if((float)(o_c[i * n + j] - c[i * n + j]) > 0.005) {
