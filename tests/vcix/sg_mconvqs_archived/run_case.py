...
CASE_NUMBER = 200
os.system("/opt/LLVM/install/bin/clang -static sg_mconvqs.c sg_mconvqs_asm.S -o sg_mconvqs -O3 -march=rv64imafdcv_zicsr_zifencei_zfh_zba_zbb_zvfh_xsfvfnrclipxfqf_xsfvfwmaccqqq_xsfvqmaccqoq_xsfvcp_xsgmat --gcc-toolchain=/opt/riscv --sysroot=/opt/riscv/sysroot/ -g")

# data configuration
executable_path = './sg_mconvqs'  # excutable file
...
for i in range(CASE_NUMBER):
    ...
    kernel = np.random.randint(-10, 10, size=(k_h, k_w), dtype=np.int8)
    input_matrix = np.random.randint(-128, 128, size=(i_h, i_w), dtype=np.int8)
    ...
    padded_input = padded_input.astype(np.int32)
    output_matrix = np.zeros((o_h, o_w), dtype=np.int32)
    for y in range(o_h):
        for x in range(o_w):
            sub_matrix = padded_input[y*strh:y*strh+k_h, x*strw:x*strw+k_w]
            conv_kernel = kernel.astype(np.int32)
