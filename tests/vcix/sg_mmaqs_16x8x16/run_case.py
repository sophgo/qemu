...
import os
CASE_NUMBER = 200
os.system("/opt/LLVM/install/bin/clang -static sg_mmaqs.c sg_mmaqs_asm.S -o sg_mmaqs -O3 -march=rv64imafdcv_zicsr_zifencei_zfh_zba_zbb_zvfh_xsfvfnrclipxfqf_xsfvfwmaccqqq_xsfvqmaccqoq_xsfvcp_xsgmat --gcc-toolchain=/opt/riscv --sysroot=/opt/riscv/sysroot/ -g")

# data configuration
matrix_size = 128  # set matrix size as 16*8=128
executable_path = './sg_mmaqs'  # excutable file
...
for i in range(CASE_NUMBER):
    ...
    a = np.random.randint(-128, 127, size=(16, 8), dtype=np.int8)
    b = np.random.randint(-128, 127, size=(8, 16), dtype=np.int8)
    a_int32 = a.astype(np.int32)
    b_int32 = b.astype(np.int32)
    c = np.dot(a_int32, b_int32).astype(np.int32)  # expected output
