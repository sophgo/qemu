...
CASE_NUMBER = 200
os.system("/opt/LLVM/install/bin/clang -static sg_mmawf.c sg_mmawf_asm.S -o sg_mmawf -O3 -march=rv64imafdcv_zicsr_zifencei_zfh_zba_zbb_zvfh_xsfvfnrclipxfqf_xsfvfwmaccqqq_xsfvqmaccqoq_xsfvcp_xsgmat --gcc-toolchain=/opt/riscv --sysroot=/opt/riscv/sysroot/ -g")

# data configuration
matrix_size = 128  # set matrix size as 16*8=128
executable_path = './sg_mmawf'  # excutable file
...
for i in range(CASE_NUMBER):
    ...
    a = np.random.uniform(-128, 128, size=(16, 8)).astype(np.float16)
    b = np.random.uniform(-128, 128, size=(8, 16)).astype(np.float16)
    a_float32 = a.astype(np.float32)
    b_float32 = b.astype(np.float32)
    c = np.dot(a_float32, b_float32).astype(np.float32)  # expected output
