...
extern void vcix_mmawf_asm(int m, int n, int k, void *a, void *b, void *c);
void matrix_multiply_and_compare(_Float16 *a, _Float16 *b, float *c);
...
int main(int argc, char **argv) {
    ...
    float *c = (float *)malloc(sizeof(float) * MATRIX_SIZE * 2);
    ...
    fread(c, sizeof(float), MATRIX_SIZE * 2, input_file);
    fclose(input_file);
...
void matrix_multiply_and_compare(_Float16 *a, _Float16 *b, float *c) {
    ...
    int n = 16;
    int k = 8;
    float *o_c = (float *)malloc(sizeof(float) * MATRIX_SIZE * 2);
    vcix_mmawf_asm(m, n, k, a, b, o_c);
    for(int i = 0; i < m; ++i) {
        for(int j = 0; j < n; ++j) {
            if(fabsf(o_c[i * n + j] - c[i * n + j]) > 0.000000001) {
